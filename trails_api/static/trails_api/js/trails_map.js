...
// Enable Proximity function
function enableProximitySearch() {
    if (typeof ProximitySearch !== 'function') {
        console.warn("⏳ ProximitySearch not yet available, retrying...");
        setTimeout(enableProximitySearch, 500);
        return;
    }

    if (!window.trailsMap) {
        console.warn("⚠️ Map not ready for proximity search, retrying...");
        setTimeout(enableProximitySearch, 500);
        return;
    }

    window.proximitySearch = new ProximitySearch(window.trailsMap);
    console.log("✅ Proximity search initialized successfully");
}
